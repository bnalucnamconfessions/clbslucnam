...
import { SIDEBAR_SHOW_BOOK_MENU, PERM_LABELS, normalizePermission, canEditCaiDat } from '@/lib/permissions'
import { logActivity } from '@/lib/activityLog'

const AUTH_KEY = 'adminToken'
const PERMISSION_POLL_INTERVAL_MS = 20 * 1000
...
export default function Sidebar() {
  ...
  const showCaiDatMenu = canEditCaiDat(userInfo.clubPermission || '')
  ...
  useEffect(() => {
    ...
    const syncFromBackend = async () => {
      try {
        ...
        if (apiEmail) {
          (updated as Record<string, unknown>).accountEmail = apiEmail
          (updated as Record<string, unknown>).email = apiEmail
        }
  ...
  const getLinkClasses = (path: string) => {
    ...
    if (active) return `${base} text-white`
    ...
    <div className="hidden lg:flex flex-col w-64 border-r bg-white h-screen overflow-y-auto no-scrollbar" style={{ borderRightColor: 'rgba(199, 199, 199, 1)' }}>
      <div className="flex flex-col px-4 md:px-6 lg:px-8 py-6 bg-white text-black">
        ...
        <div className="flex flex-col gap-2">
          ...
          {showBookMenu && (
            <>
              <Link className={getLinkClasses('/books')} style={isActive('/books') ? { backgroundColor: '#137fec' } : {}} to="/books">
              ...
              <Link className={getLinkClasses('/muon')} style={isActive('/muon') ? { backgroundColor: '#137fec' } : {}} to="/muon">
              ...
              <Link className={getLinkClasses('/tra')} style={isActive('/tra') ? { backgroundColor: '#137fec' } : {}} to="/tra">
          ...
          {showOverviewMenu && (
            <>
              <Link className={getLinkClasses('/thanh-vien')} style={isActive('/thanh-vien') ? { backgroundColor: '#137fec' } : {}} to="/thanh-vien">
              ...
              <Link className={getLinkClasses('/dashboard/tai-chinh')} style={isActive('/dashboard/tai-chinh') ? { backgroundColor: '#137fec' } : {}} to="/dashboard/tai-chinh">
          ...
          <Link className={getLinkClasses('/dashboard/xep-hang')} style={isActive('/dashboard/xep-hang') ? { backgroundColor: '#137fec' } : {}} to="/dashboard/xep-hang">
          ...
          {showDoiTacMenu && (
            <Link className={getLinkClasses('/dashboard/doi-tac')} style={isActive('/dashboard/doi-tac') ? { backgroundColor: '#137fec' } : {}} to="/dashboard/doi-tac">
          ...
          {showCaiDatMenu && (
            <Link className={getLinkClasses('/dashboard/cai-dat')} style={pathname.startsWith('/dashboard/cai-dat') ? { backgroundColor: '#137fec' } : {}} to="/dashboard/cai-dat/website">
              <span className={`material-symbols-outlined ${pathname.startsWith('/dashboard/cai-dat') ? 'fill-1' : ''}`}>settings</span>
              <span className="text-sm font-medium leading-normal">Cài đặt</span>
            </Link>
          )}
          <Link className={getLinkClasses('/ho-so')} style={isActive('/ho-so') ? { backgroundColor: '#137fec' } : {}} to="/ho-so">

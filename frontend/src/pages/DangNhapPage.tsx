...
export default function DangNhapPage() {
  ...
  useEffect(() => {
    ...
    if (token) {
      ...
      if (typeof token === 'string' && token.startsWith('google-') && (email || '').trim()) logActivity('Đăng nhập', `Đăng nhập qua Google | Email: ${(email || '').trim() || '—'}`, (email || '').trim() || undefined)
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(apiUrl('/api/auth/register'), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email: registerEmail.trim(), password }),
      })
      const data = await res.json().catch(() => ({}))
      if (res.ok && data.sent) {
        setError('')
  ...
  const handleVerifyRegister = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(apiUrl('/api/auth/register/verify'), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email: registerEmail.trim(), code, password }),
      })
      const data = await res.json().catch(() => ({}))
      if (res.ok) {
              ...
              <div className="flex h-full flex-col items-start justify-center gap-8 lg:flex-row lg:gap-12">
                ...
                <div className="order-1 mx-auto w-full max-w-[520px] lg:order-2 lg:mx-0">
                  <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-xl sm:p-8">
                    ...
                    {activeTab === 'login' ? (
                      <form onSubmit={handleLogin} className="flex flex-col gap-4">
                        <label className="flex flex-col gap-1.5">
                          ...
                          <div className="relative flex items-center">
                            ...
                            <input type="text" value={username} onChange={(e) => setUsername(e.target.value)} className="h-12 w-full rounded-lg border border-slate-200 bg-slate-50 pl-11 pr-4 text-sm text-slate-900 placeholder-slate-400/50 transition-all focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary" placeholder="Nhập tài khoản" required autoComplete="username" />
                          </div>
                        </label>
                        <label className="flex flex-col gap-1.5">
                          ...
                          <div className="relative flex items-center">
                            ...
                            <input type={showPassword ? 'text' : 'password'} value={password ?? ''} onChange={(e) => setPassword(e.target.value)} className="h-12 w-full rounded-lg border border-slate-200 bg-slate-50 pl-11 pr-12 text-sm text-slate-900 placeholder-slate-400/50 transition-all focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary" placeholder="Ít nhất 8 ký tự" required autoComplete="current-password" />
                        ...
                        <div className="flex justify-end">
                          <Link to="/quen-mat-khau" className="text-sm text-[#137fec] hover:underline font-medium">Quên mật khẩu?</Link>
                        </div>
                        {successMessage && <p className="flex items-center gap-2 text-sm text-green-600"><span className="material-symbols-outlined text-lg">check_circle</span>{successMessage}</p>}
                        ...
                        <button type="submit" disabled={loading} className="mt-4 flex h-12 w-full items-center justify-center rounded-lg bg-[#137fec] text-base font-bold text-white shadow-lg shadow-[#137fec]/20 transition-all hover:bg-[#0f6fd6] active:scale-[0.98] disabled:opacity-70">
                      ...
                      <form onSubmit={handleVerifyRegister} className="flex flex-col gap-4">
                        {alreadyGoogleMessage && (
                          <div className="flex flex-col gap-1 rounded-lg border border-blue-200 bg-blue-50 p-3 text-sm text-blue-800">
                            <p className="flex items-center gap-2 font-medium"><span className="material-symbols-outlined text-lg">info</span>{alreadyGoogleMessage}</p>
                            <p className="text-blue-700">Dùng nút &quot;Tiếp tục với Google&quot; bên dưới để đăng nhập.</p>
                          </div>
                        )}
                        <p className="text-sm text-slate-600">Mã 6 chữ số đã gửi đến <strong className="text-slate-900">{registerEmail}</strong>. Nhập mã bên dưới để hoàn tất đăng ký.</p>
                        <label className="flex flex-col gap-1.5">
                          ...
                          <div className="relative flex items-center">
                            ...
                            <input type="text" inputMode="numeric" maxLength={6} value={verificationCode} onChange={(e) => setVerificationCode(e.target.value.replace(/\D/g, '').slice(0, 6))} className="h-12 w-full rounded-lg border border-slate-200 bg-slate-50 pl-11 pr-4 text-lg tracking-[0.4em] font-mono text-slate-900 text-center transition-all focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary" placeholder="000000" autoComplete="one-time-code" />
                          </div>
                        </label>
                        {error && <p className="flex items-center gap-2 text-sm text-red-600"><span className="material-symbols-outlined text-lg">error</span>{error}</p>}
                        <button type="submit" disabled={loading || verificationCode.replace(/\D/g, '').length !== 6} className="mt-4 flex h-12 w-full items-center justify-center rounded-lg bg-[#137fec] text-base font-bold text-white shadow-lg shadow-[#137fec]/20 transition-all hover:bg-[#0f6fd6] active:scale-[0.98] disabled:opacity-70">
                        ...
                        <button type="button" onClick={() => { setRegisterStep('form'); setError(''); setSuccessMessage(''); setAlreadyGoogleMessage(''); setVerificationCode('') }} className="text-sm text-slate-500 hover:text-primary">← Quay lại nhập email</button>
                      </form>
                    ) : (
                      <form onSubmit={handleRegister} className="flex flex-col gap-4">
                        <label className="flex flex-col gap-1.5">
                          ...
                          <div className="relative flex items-center">
                            <span className="material-symbols-outlined absolute left-4 text-[20px] text-slate-500">mail</span>
                            <input type="email" value={registerEmail ?? ''} onChange={(e) => { setRegisterEmail(e.target.value); setAlreadyGoogleMessage('') }} className="h-12 w-full rounded-lg border border-slate-200 bg-slate-50 pl-11 pr-4 text-sm text-slate-900 placeholder-slate-400/50 transition-all focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary" placeholder="tài khoản@gmail.com" required autoComplete="email" />
                          </div>
                        </label>
                        <label className="flex flex-col gap-1.5">
                          <span className="text-sm font-medium text-slate-900">Mật khẩu</span>
                          <div className="relative flex items-center">
                            <span className="material-symbols-outlined absolute left-4 text-[20px] text-slate-500">lock</span>
                            <input type={showPassword ? 'text' : 'password'} value={password} onChange={(e) => setPassword(e.target.value)} className="h-12 w-full rounded-lg border border-slate-200 bg-slate-50 pl-11 pr-12 text-sm text-slate-900 placeholder-slate-400/50 transition-all focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary" placeholder="Ít nhất 8 ký tự" required minLength={8} />
                            <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-4 flex items-center text-slate-500 hover:text-primary"><span className="material-symbols-outlined text-[20px]">{showPassword ? 'visibility' : 'visibility_off'}</span></button>
                          </div>
                        </label>
                        <label className="flex flex-col gap-1.5">
                          ...
                          <div className="relative flex items-center">
                            ...
                            <input type={showPassword ? 'text' : 'password'} value={confirmPassword ?? ''} onChange={(e) => setConfirmPassword(e.target.value)} className="h-12 w-full rounded-lg border border-slate-200 bg-slate-50 pl-11 pr-4 text-sm text-slate-900 placeholder-slate-400/50 transition-all focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary" placeholder="Nhập lại mật khẩu" />
                          </div>
                        </label>
                        <label className="mt-2 flex cursor-pointer select-none items-start gap-3 group">
                          <input type="checkbox" checked={agreeTerms} onChange={(e) => setAgreeTerms(e.target.checked)} className="mt-1 h-5 w-5 rounded border border-slate-200 text-primary focus:ring-primary" />
                          <span className="text-sm leading-snug text-slate-500 transition-colors group-hover:text-primary">Tôi đồng ý cho phép sử dụng thông tin cá nhân để mượn sách và nhận quà tặng từ câu lạc bộ.</span>
                        </label>
                        {alreadyGoogleMessage && (
                          <div className="flex flex-col gap-1 rounded-lg border border-blue-200 bg-blue-50 p-3 text-sm text-blue-800">
                            <p className="flex items-center gap-2 font-medium"><span className="material-symbols-outlined text-lg">info</span>{alreadyGoogleMessage}</p>
                    ...
                    <a href={apiUrl('/api/auth/google/start')} className="flex w-full items-center justify-center gap-3 rounded-lg border border-slate-200 bg-white h-12 px-4 hover:bg-slate-50 transition-colors text-slate-900 font-bold text-sm">
                    ...
                    <p className="mt-6 text-center text-xs text-slate-500">
                      Bằng cách đăng ký, bạn đồng ý với <Link to="#" className="text-primary hover:underline">Điều khoản sử dụng</Link> và <Link to="#" className="text-primary hover:underline">Chính sách bảo mật</Link> của chúng tôi.
                    </p>
                  </div>
                </div>
              </div>

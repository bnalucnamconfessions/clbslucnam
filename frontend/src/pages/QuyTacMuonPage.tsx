...
export default function QuyTacMuonPage() {
  ...
  useEffect(() => {
    ...
    fetch(apiUrl('/api/website-config'), { credentials: 'include' })
      ...
      .then((data: { borrowRules?: BorrowRules }) => {
  ...
  const handleSave = async () => {
    ...
    try {
      const res = await fetch(apiUrl('/api/website-config/update'), {
        ...
        body: JSON.stringify({ accountEmail: accountEmail || undefined, borrowRules }),
      })
      if (!res.ok) {
        ...
        <div className="flex-1 overflow-y-auto scroll-smooth bg-white no-scrollbar">
          <header className="px-4 md:px-6 lg:px-8 pt-6 pb-6 border-b border-slate-200 bg-white">
            <div className="flex flex-col gap-2">
              ...
              <p className="text-slate-500 text-base font-normal leading-normal">Chỉ Ban chủ nhiệm có quyền chỉnh sửa hạn trả dự kiến và số sách tối đa theo từng vai trò.</p>
            </div>
          </header>
          <div className="px-4 md:px-6 lg:px-8 py-6 w-full">
            <div className="flex items-center gap-2 mb-6 border-b border-slate-200 pb-2">
              <Link to="/dashboard/cai-dat/website" className="px-4 py-2 rounded-lg text-sm font-bold transition-all text-slate-600 hover:bg-slate-100">Cấu hình website</Link>
              <span className="px-4 py-2 rounded-lg text-sm font-bold transition-all bg-[#137fec] text-white shadow-sm">Quy tắc mượn sách</span>
            </div>
                          ...
                          <tr key={role} className="border-t border-slate-100">
                            ...
                            <td className="px-3 py-2">
                              <input
                                ...
                                onChange={(e) => {
                                  ...
                                  if (!Number.isNaN(v)) setBorrowRules((prev) => ({ ...prev, [role]: { ...prev[role], dueDays: Math.max(1, Math.min(365, v)), maxBooks: prev[role]?.maxBooks ?? 3 } }))
                                }}
                            ...
                            <td className="px-3 py-2">
                              <input
                                ...
                                onChange={(e) => {
                                  const v = parseInt(e.target.value, 10)
                                  if (!Number.isNaN(v)) setBorrowRules((prev) => ({ ...prev, [role]: { dueDays: prev[role]?.dueDays ?? 14, maxBooks: Math.max(1, Math.min(20, v)) } }))
                                }}
                                className="w-20 px-2 py-1.5 border border-slate-300 rounded focus:ring-2 focus:ring-[#137fec]"
                              />
                            </td>

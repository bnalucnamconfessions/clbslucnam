        ...
        <div className="flex-1 overflow-y-auto scroll-smooth bg-white no-scrollbar flex items-center justify-center px-4 py-12">
          <div className="w-full max-w-md">
            <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-xl sm:p-8">
              ...
              <p className="text-sm text-slate-500 mb-6">Nhập email đã đăng ký. Chúng tôi sẽ gửi link đặt lại mật khẩu qua email.</p>
              <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                <label className="flex flex-col gap-1.5">
                  <span className="text-sm font-medium text-slate-900">Email</span>
                  <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="h-12 w-full rounded-lg border border-slate-200 bg-slate-50 px-4 text-sm text-slate-900 placeholder-slate-400 focus:border-[#137fec] focus:outline-none focus:ring-1 focus:ring-[#137fec]" placeholder="tài khoản@gmail.com" required autoComplete="email" />
                </label>
                {message && (
                  <div className="space-y-2">
                    <p className="flex items-center gap-2 text-sm text-green-600"><span className="material-symbols-outlined text-lg">check_circle</span>{message}</p>
                    {debugResetUrl && <p className="text-sm"><a href={debugResetUrl} className="text-[#137fec] underline font-medium">Nhấn vào đây để đặt lại mật khẩu</a></p>}
                    <p className="text-xs text-slate-500">Nếu không thấy email, hãy kiểm tra thư mục <strong>Spam</strong> hoặc <strong>Rác</strong>.</p>
                  </div>
                )}
